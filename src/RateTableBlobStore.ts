...
export class RateTableBlobStore implements IRateTableStore {
    ...
    constructor(account, key, container=null) 
    {
        this._account = account;
        this._accountKey = key;

        this._sharedKeyCredential = new SharedKeyCredential(this._account, this._accountKey)
        this._pipeline = Azure.StorageURL.newPipeline(this._sharedKeyCredential);
        this._serviceURL = new Azure.ServiceURL(
    ...
    private async getContainer(name: string) : Promise<string> {
        ...
        const containerURL = Azure.ContainerURL.fromServiceURL(this._serviceURL, containerName);

        const result = await this._serviceURL.listContainersSegment(
            Azure.Aborter.none,
            undefined,
            {
              include: "metadata",
